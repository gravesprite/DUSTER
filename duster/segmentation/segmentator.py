...
class Segmentator:
    ...
    def __init__(self, velodyne_dir, pc_files, queries, logger, poses, Tr, threshold, args, max_length, A,
                 max_processes=4, exploration_param=1.0, hw_config=None):
        """
        Parameters:
          ...
          hw_config: HardwareConfig instance (auto-detected if None).
        """
    ...
    def direct_assign_segments(self, pc_files, segment_counts):
        ...
        T = self.hw.block_parallel
        batch_segments = []
        gpu_ids = itertools.cycle(self.hw.gpu_ids)
    ...
    def assign_segments_with_gaussian_smoothing(self, pc_files, segment_counts):
        ...
        T = self.hw.block_parallel  # Number of blocks to process in parallel
        ...
        while num_long > 0 and not all(selected_mask):
            ...
            # Parallel evaluation of blocks — uses scheduler config
            gpu_ids = itertools.cycle(self.hw.gpu_ids)
            args_with_gpu = [
                (
        ...
        tasks = []
        gpu_ids = itertools.cycle(self.hw.gpu_ids)
        MAX_WORKERS = self.hw.worker_parallel
